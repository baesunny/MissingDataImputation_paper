...
고비용·장기 누적 실험에서 흔한 **불완전 공정 데이터**의 독립변수 결측을, **MICE · missForest · 1D-CNN** 세 가지 대표 기법으로 대체·비교한 학술 논문 및 재현 코드 저장소이다. 친환경 슬래그 시멘트(초속경성) 제조 현장에서 수집된 혼합형(연속·범주) 공정 변수를 대상으로, CTQ 예측에 앞서 결측 대체 품질을 실증한다.

**프로젝트 상태: 완료** — 논문 게재본(JKSIE Vol.48 No.4)과 동일한 분석·대체·시각화 파이프라인이 마감된 상태이다.

**논문 저자:** 배재호(오산대학교) · 최선미 · **배성윤**(국민대학교 AI빅데이터융합경영학과)
...
본 README와 `notebooks/`, `scripts/`, `R/` 코드는 위 논문의 실험 절차(데이터 전처리, MICE/missForest/1D-CNN 대체, 오차·분포 비교)를 재현하기 위해 구성되었다.
...
**절차:** 엑셀/CSV 로드 → 독립·종속·식별 변수 지정 → (선택) Class1/Class2 필터 → 결측 대체 → Excel·PDF·오차 CSV 저장
...
# 통합 대화형 스크립트 (sources/ 아래 xlsx 필요)
...
노트북은 각 파일이 있는 디렉터리를 작업 경로로 두고 실행하는 것이 좋다(`../../data/` 상대 경로 기준).
...
- 포트폴리오 공개를 위해 **기업 식별이 가능한 원시 엑셀**(`CSM_25617_JHBae.xlsx` 등)은 저장소에서 제거하였다.
- 재현은 `data/Imputation_Test.xlsx` 및 파생 CSV만으로 가능하도록 경로를 정리하였다.
- 논문·발표 PDF는 학술·설명 목적으로 `docs/`에 유지한다.
